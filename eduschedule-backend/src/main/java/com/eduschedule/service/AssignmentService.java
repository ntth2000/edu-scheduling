...
public class AssignmentService {
        ...
        public AssignmentResponse assign(AssignmentRequest request) {
                ...
                int currentPeriods = assignmentRepository
                                .countTotalPeriodsByTeacher(teacher.getId());
                int newPeriods = getPeriodsForGrade(
                                subject, schoolClass.getGrade());
                if (currentPeriods + newPeriods > teacher.getMaxPeriodsPerWeek()) {
                        throw new RuntimeException(
                                        "Giáo viên " + teacher.getFullName() +
                                                        " vượt định mức " +
                                                        teacher.getMaxPeriodsPerWeek() + " tiết/tuần");
                }
